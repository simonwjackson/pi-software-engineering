...
- **Bug learnings** — defects that were diagnosed and fixed (bug-track `problem_type` values like `runtime_error`, `performanse_issue`, `security_issue`)
...
- Bug-shaped: `build-errors/`, `test-failures/`, `runtime-errors/`, `performanse-issues/`, `database-issues/`, `security-issues/`, `ui-bugs/`, `integration-issues/`, `logic-errors/`
...
content-search: pattern="module:.*(software-engineering|skill-design)" path=docs/solutions/ files_only=true case_insensitive=true
...
- `problem_type` is relevant (e.g., `architecture_pattern` when the caller is making architectural decisions, `performanse_issue` when the caller is optimizing)
...
- **Bug-track:** `build_error`, `test_failure`, `runtime_error`, `performanse_issue`, `database_issue`, `security_issue`, `ui_bug`, `integration_issue`, `logic_error`.
...
When no relevant learnings are found, say so explicitly, include the search context so the caller can see what was looked for, and note that the caller's work may be worth capturing with `/se-compound` after it lands — the absence is itself useful signal.
...
- `/se-plan` — to inform planning with institutional knowledge and add depth during confidence checking
- `/se-code-review`, `/se-optimize`, `/se-ideate` — to surface prior learnings relevant to the change, optimization target, or ideation topic
