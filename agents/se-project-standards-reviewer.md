---
name: se-project-standards-reviewer
...
2. For each changed file, check its ancestor directories up to the repo root for standards files. A file like `plugins/software-engineering/AGENTS.md` applies to all changes under `plugins/software-engineering/`.
...
- **Broken cross-references** -- agent names that are not fully qualified (e.g., `se-learnings-researcher` instead of `se-learnings-researcher`). Skill-to-skill references using slash syntax inside a SKILL.md where the standards say to use semantic wording. References to tools by platform-specific names without naming the capability class.
...
- **Protected artifact violations** -- findings, suggestions, or instructions that recommend deleting or gitignoring files in paths the standards designate as protected (e.g., `work/items/active/*/requirements.md`, `work/items/active/*/plan.md`, `work/items/parking-lot/`, `docs/solutions/`).
