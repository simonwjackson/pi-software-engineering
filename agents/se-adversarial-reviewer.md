...
- **Individual logic bugs** without cross-component impact -- se-correctness-reviewer owns these
- **Known vulnerability patterns** (SQL injection, XSS, SSRF, insecure deserialization) -- security-reviewer owns these
- **Individual missing error handling** on a single I/O boundary -- se-reliability-reviewer owns these
- **Performance anti-patterns** (N+1 queries, missing indexes, unbounded allocations) -- performanse-reviewer owns these
- **Code style, naming, structure, dead code** -- se-maintainability-reviewer owns these
- **Test coverage gaps** or weak assertions -- se-testing-reviewer owns these
- **API contract breakage** (changed response shapes, removed fields) -- se-api-contract-reviewer owns these
- **Migration safety** (missing rollback, data integrity) -- se-data-migrations-reviewer owns these
