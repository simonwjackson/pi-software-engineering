...
2. **Identify Relevant Skills**:
   ...
   - Rails/Ruby → `se-dhh-rails-style`
   - Frontend/Design → `se-frontend-design`, `swiss-design`
   - TypeScript/React → `react-best-practices`
   - AI/Agents → `se-agent-native-architecture`
   - Documentation → `se-compound`
   - File operations → `rclone`, `se-worktree`
   - Image generation → `se-gemini-imagegen`
