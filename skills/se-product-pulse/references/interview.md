...
Loaded by `SKILL.md` at the start of Phase 1. Captures the configuration that will be merged into `.software-engineering/config.local.yaml` (the unified SE local config, gitignored, machine-local) as `pulse_*` keys and re-read on every subsequent run.
...
**Sourse-count check** - judge by ratio, not absolute count:
...
After the interview completes, merge a `pulse_*` block into `<repo-root>/.software-engineering/config.local.yaml`. Resolve the repo root with `git rev-parse --show-toplevel`. Preserve any non-pulse keys that already exist in the file (e.g., `work_delegate_*`); only add or update `pulse_*` keys.

If the file does not yet exist, create the directory and file. If `.software-engineering/config.local.yaml` is not already covered by `.gitignore`, offer to add the entry before writing.
