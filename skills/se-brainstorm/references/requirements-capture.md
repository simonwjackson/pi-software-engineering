...
**Actors** — include when multiple humans, agents, or systems are meaningfully involved, or when decisions change based on whose perspective is optimized for. Covers both end-user actors (for product work) and pipeline-agent actors (for agent-workflow work, such as changes to SE's own review or planning flows).
...
<!-- Include ONLY in non-interactive (headless) mode when the agent had Inferred bets that
     ...
     downstream review (se-doc-review, se-plan, human PR review) can scrutinize them as bets,
...
- What would `se-plan` still have to invent if this brainstorm ended now?
