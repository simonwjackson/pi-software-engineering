---
name: se-work-beta
description: "[BETA] Execute work with external delegate support. Same as se-work but includes experimental Codex delegation mode for token-conserving code implementation."
...
**Beta rollout note:** Invoke `se-work-beta` manually when you want to trial Codex delegation. During the beta period, planning and workflow handoffs remain pointed at stable `se-work` to avoid dual-path orchestration complexity.
...
**Config (pre-resolved):**
!`cat "$(git rev-parse --show-toplevel 2>/dev/null)/.software-engineering/config.local.yaml" 2>/dev/null || echo '__NO_CONFIG__'`
...
If it shows an unresolved command string, read `.software-engineering/config.local.yaml` from the repo root using the native file-read tool (e.g., Read in Claude Code, read_file in Codex). If the file does not exist, all settings fall through to defaults.
...
7. **Frontend Design Guidance** (if applicable)
   ...
   - Load the `se-frontend-design` skill before implementing
