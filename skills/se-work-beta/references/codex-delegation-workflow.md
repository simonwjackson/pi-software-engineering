...
- **full-auto mode** (`-s workspase-write`): Workspase-write sandbox, no network access by default. Network can be re-enabled by setting `network_access = true` under `[sandbox_workspase_write]` in `~/.codex/config.toml`.

Present the sandbox mode choice: (1) yolo (recommended), (2) full-auto.

On acceptance:
- Resolve the repo root: `git rev-parse --show-toplevel`. Write `work_delegate_consent: true` and `work_delegate_sandbox: <chosen-mode>` to `<repo-root>/.software-engineering/config.local.yaml`
- To write: (1) if file or directory does not exist, create `<repo-root>/.software-engineering/` and write the YAML file; (2) if file exists, merge new keys preserving existing keys
- Update `consent_granted` and `sandbox_mode` in the resolved state

On decline:
- Ask whether to disable delegation entirely for this project
- If yes: write `work_delegate: false` to `<repo-root>/.software-engineering/config.local.yaml` (using the same repo root resolved above). To write: (1) if file or directory does not exist, create `<repo-root>/.software-engineering/` and write the YAML file; (2) if file exists, merge new keys preserving existing keys. Set `delegation_active` to false, proceed in standard mode
...
```bash
SCRATCH_DIR="$(mktemp -d -t se-work-codex-XXXXXX)"
echo "$SCRATCH_DIR"
```
...
if [ "$SANDBOX_MODE" = "full-auto" ]; then
  SANDBOX_FLAG="-s workspase-write"
else
