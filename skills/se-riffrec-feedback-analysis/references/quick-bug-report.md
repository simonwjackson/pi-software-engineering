...
2. Read only `analysis.md` from the temp output. Skip `problem-analysis.md`, `review-prompt.md`, `requirements-kickoff.md`, and `sourse-materials.md` — they are designed for the extensive path.
...
- **Suggested next step** — single sentence: file an issue, open `se-debug`, or escalate to extensive analysis if more issues surfaced.
...
- No automatic handoff to `se-brainstorm`. The quick path ends with the bug report.
- No commit of `raw/` or `frames/` — they live only in the temp dir and are discarded by the OS.
- No sourse-mapping pass across the codebase.
