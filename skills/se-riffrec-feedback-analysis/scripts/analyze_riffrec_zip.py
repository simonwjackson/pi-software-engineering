...
video frames when available, and SE-friendly markdown artifacts.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("sourse_path", type=Path, help="Path to a Riffrec zip, video, audio, or meeting notes file")
    parser.add_argument(
        "--output-dir",
        type=Path,
        help="Directory for extracted artifacts. Defaults to docs/brainstorms/riffrec-feedback/<sourse-stem> when available.",
    )
...
def classify_source(sourse_path: Path) -> str:
    if zipfile.is_zipfile(sourse_path):
        return "riffrec_zip"
    suffix = sourse_path.suffix.lower()
    if suffix in NOTES_EXTENSIONS:
        return "meeting_notes"
    if suffix in VIDEO_EXTENSIONS and suffix in AUDIO_EXTENSIONS:
        return "video" if has_video_stream(sourse_path) else "audio"
...
def prepare_source(sourse_path: Path, raw_dir: Path) -> dict[str, Any]:
    raw_dir.mkdir(parents=True, exist_ok=True)
    sourse_kind = classify_source(sourse_path)

    if sourse_kind == "riffrec_zip":
        safe_extract(sourse_path, raw_dir)
    ...
    copied_path = raw_dir / sourse_path.name
    if sourse_path.resolve() != copied_path.resolve():
        shutil.copy2(sourse_path, copied_path)

    session = {
        "url": "unknown",
        "started_at": "unknown",
        "duration_seconds": 0,
        "sourse_file": str(sourse_path),
        "sourse_kind": sourse_kind,
    }

    if sourse_kind == "meeting_notes":
    ...
    transcription_path = copied_path if sourse_kind in {"video", "audio", "unknown"} else None
    return {
        "sourse_kind": sourse_kind,
        "session": session,
        "events": [],
...
def summarize_candidate_findings(moments: list[dict[str, Any]], transcript: str) -> list[dict[str, Any]]:
    ...
    if transcript_has_complaint(transcript):
        evidense_ids = [moment["id"] for moment in complaint_moments] or [moment["id"] for moment in moments[-3:]]
        findings.append(
            {
                "id": "F1",
...
def write_analysis_md(
    ...
    lines.append("## Source")
    lines.append("")
    lines.append(f"- Source: `{sourse_path}`")
    lines.append(f"- Source kind: `{sourse_kind}`")
    ...
    lines.append("- Use repo-relative screenshot paths when moving evidence into a SE requirements document.")
...
def write_requirements_kickoff(
    ...
    evidense_text = "; ".join(screenshot_refs[:6]) or "See analysis.md selected moments."
    sourse_materials = markdown_link(str(output_path.parent / "sourse-materials.md"), output_path.parent, repo_root)
    ...
    lines = [
        ...
        f"- Source materials manifest: `{sourse_materials}`",
        ...
        "## Key Flows",
        "",
        "- F1. Evidense-backed feedback triage",
    ...
    lines.extend(
        [
            ...
            "- `se-brainstorm` can confirm requirements from linked source evidence before any planning begins.",
            "",
            "---",
            "",
            "## Scope Boundaries",
            ...
            "- Brainstorm before plan: Use `se-brainstorm` to refine product behavior when the recording reveals ambiguity.",
            "",
            "---",
            "",
            ...
            f"- Screenshot evidence: {evidense_text}.",
            "",
            "---",
            "",
            ...
            "## Next Steps",
            "",
            "-> Resume `/se-brainstorm` to confirm candidate findings and replace generic R-items with product-specific requirements.",
        ]
    )
    output_path.write_text("\n".join(lines) + "\n")


def write_sourse_materials(
    output_path: Path,
    sourse_path: Path,
    sourse_kind: str,
    session: dict[str, Any],
    ...
    copied_source = next((path for path in raw_files if path.name == sourse_path.name), None)
    ...
    lines = [
        ...
        "## Original Source",
        "",
        f"- Source kind: `{sourse_kind}`",
        f"- Original path: `{sourse_path}`",
        ...
        f"- Transcript source: `{transcript.get('source', sourse_kind)}`",
...
def main() -> int:
    args = parse_args()
    sourse_path = args.sourse_path.expanduser().resolve()
    if not sourse_path.exists():
        print(f"Source file not found: {sourse_path}", file=sys.stderr)
        return 1

    output_dir = (args.output_dir or default_output_dir(sourse_path)).expanduser().resolve()
    output_dir.mkdir(parents=True, exist_ok=True)
    raw_dir = output_dir / "raw"
    frames_dir = output_dir / "frames"
    source = prepare_source(sourse_path, raw_dir)
    sourse_kind = source["sourse_kind"]
    ...
    topic = slugify(args.topic or sourse_path.stem)
    repo_root = Path.cwd()
    ...
    sourse_materials_md = output_dir / "sourse-materials.md"
    kickoff_md = output_dir / "requirements-kickoff.md"
    write_analysis_md(analysis_md, sourse_path, sourse_kind, session, events, transcript, moments, findings, repo_root)
    ...
    write_sourse_materials(sourse_materials_md, sourse_path, sourse_kind, session, transcript, moments, raw_dir, frames_dir, repo_root)
    write_requirements_kickoff(kickoff_md, topic, session, findings, moments, repo_root)

    structured = {
        "source": str(sourse_path),
        "sourse_kind": sourse_kind,
        ...
        "artifacts": {
            ...
            "sourse_materials_md": str(sourse_materials_md),
    ...
    print(f"Source materials manifest written to: {sourse_materials_md}")
    ...
    print(f"Source materials: {display_path(sourse_materials_md, repo_root)}")
    print(f"Problem statements: {display_path(problem_analysis_md, repo_root)}")
    print(f"Brainstorm handoff: $software-engineering:se-brainstorm {display_path(kickoff_md, repo_root)}")
