...
- **recommended next step:** `/se-brainstorm` (shown in the proof skill's final terminal output)
...
- **§6.3 Brainstorm a selected idea.** On a successful Proof return (`proceeded` or `done_for_now`), do **not** stop at the Phase 6 menu — after applying the per-status handling below (including any stale-local pull offer), continue into §6.3's remaining bullets (mark the chosen idea as `Explored`, then load `se-brainstorm`). Only the `aborted` branch returns to the Phase 6 menu, since no durable record was written.
...
- Load the `se-brainstorm` skill with the chosen idea as the seed

**Repo mode only:** do **not** skip brainstorming and go straight to `se-plan` from ideation output — `se-plan` wants brainstorm-grounded requirements. In elsewhere modes, ideation (or ideation + Proof iteration) is a legitimate terminal state; brainstorming is optional deeper development of one idea, not a required next rung on an implementation ladder that does not exist in these modes.
...
The `se-proof` skill performs single-retry-once internally on transient failures (`STALE_BASE`, `BASE_TOKEN_REQUIRED`) before surfacing failure. The proof skill's return contract does not expose typed error classes to callers — the orchestrator cannot distinguish retryable vs terminal failures from outside.
...
- acting on an idea routes to `se-brainstorm`, not directly to implementation
