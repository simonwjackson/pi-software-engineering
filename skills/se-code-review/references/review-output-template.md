...
- **Learnings & Past Solutions section** -- results from se-learnings-researcher, with links to docs/solutions/ files
- **Agent-Native Gaps section** -- results from se-agent-native-reviewer. Omit if no gaps found.
- **Schema Drift Check section** -- results from se-schema-drift-detector. Omit if the agent did not run.
- **Deployment Notes section** -- key checklist items from se-deployment-verification-agent. Omit if the agent did not run.
