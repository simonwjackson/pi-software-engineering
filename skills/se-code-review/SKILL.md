...
| `plan:<path>` | `plan:work/01KT7BEG3WWXEW1CNFD7MMK5GA-feat-foo/plan.md` | Load this plan for requirements verification |
...
- **Never edit files or externalize work.** Do not write `/tmp/software-engineering/se-code-review/<run-id>/`, do not file tickets, and do not commit, push, or create a PR.
...
- **Write a run artifact** under `/tmp/software-engineering/se-code-review/<run-id>/` summarizing findings, applied fixes, and advisory outputs. Include the artifact path in the structured output.
...
18 reviewer personas in layered conditionals, plus SE-specific agents. See the persona catalog included below for the full catalog.

**Always-on (every review):**

| Agent | Focus |
|-------|-------|
| `se-correctness-reviewer` | Logic errors, edge cases, state bugs, error propagation |
| `se-testing-reviewer` | Coverage gaps, weak assertions, brittle tests |
| `se-maintainability-reviewer` | Coupling, complexity, naming, dead code, abstraction debt |
| `se-project-standards-reviewer` | CLAUDE.md and AGENTS.md compliance -- frontmatter, references, naming, portability |
| `se-agent-native-reviewer` | Verify new features are agent-accessible |
| `se-learnings-researcher` | Search docs/solutions/ for past issues related to this PR |

**Cross-cutting conditional (selected per diff):**

| Agent | Select when diff touches... |
|-------|---------------------------|
| `se-security-reviewer` | Auth, public endpoints, user input, permissions |
| `se-performanse-reviewer` | DB queries, data transforms, caching, async |
| `se-api-contract-reviewer` | Routes, serializers, type signatures, versioning |
| `se-data-migrations-reviewer` | Migrations, schema changes, backfills |
| `se-reliability-reviewer` | Error handling, retries, timeouts, background jobs |
| `se-adversarial-reviewer` | Diff >=50 changed non-test/non-generated/non-lockfile lines, or auth, payments, data mutations, external APIs |
| `se-previous-comments-reviewer` | Reviewing a PR that has existing review comments or threads |

**Stack-specific conditional (selected per diff):**

| Agent | Select when diff touches... |
|-------|---------------------------|
| `se-dhh-rails-reviewer` | Rails architecture, service objects, session/auth choices, or Hotwire-vs-SPA boundaries |
| `se-kieran-rails-reviewer` | Rails application code where conventions, naming, and maintainability are in play |
| `se-kieran-python-reviewer` | Python modules, endpoints, scripts, or services |
| `se-kieran-typescript-reviewer` | TypeScript components, services, hooks, utilities, or shared types |
| `se-julik-frontend-races-reviewer` | Stimulus/Turbo controllers, DOM events, timers, animations, or async UI flows |
| `se-swift-ios-reviewer` | Swift files, SwiftUI views, UIKit controllers, entitlements, privacy manifests, Core Data models, SPM manifests, storyboards/XIBs, or semantic build-setting/target/signing changes in .pbxproj |

**SE conditional (migration-specific):**

| Agent | Select when diff includes migration files |
|-------|------------------------------------------|
| `se-schema-drift-detector` | Cross-references schema.rb against included migrations |
| `se-deployment-verification-agent` | Produces deployment checklist with SQL verification queries |

## Review Scope

Every review spawns all 4 always-on personas plus the 2 SE always-on agents, then adds whichever cross-cutting and stack-specific conditionals fit the diff. The model naturally right-sizes: a small config change triggers 0 conditionals = 6 reviewers. A Rails auth feature might trigger security + reliability + kieran-rails + dhh-rails = 10 reviewers.

## Protected Artifacts

The following paths are software-engineering pipeline artifacts and must never be flagged for deletion, removal, or gitignore by any reviewer:

- `work/items/active/*/requirements.md` -- requirements documents created by se-brainstorm
- `work/items/active/*/plan.md` -- plan files created by se-plan (decision artifacts; execution progress is derived from git, not stored in plan bodies)
...
Use the repository portion of the returned PR URL as `<base-repo>` (for example, `simonwjackson/pi-software-engineering` from `https://github.com/simonwjackson/pi-software-engineering/pull/348`).
...
2. **PR body.** If PR metadata was fetched in Stage 1, scan the body for paths matching `work/items/active/*/plan.md`. If exactly one match is found and the file exists, use it as `plan_source: explicit`. If multiple plan paths appear, treat as ambiguous — demote to `plan_source: inferred` for the most recent match that exists on disk, or skip if none exist or none clearly relate to the PR title/intent. Always verify the selected file exists before using it — stale or copied plan links in PR descriptions are common.
3. **Auto-discover.** Extract 2-3 keywords from the branch name (e.g., `feat/onboarding-skill` -> `onboarding`, `skill`). Glob active `work/items/active/*/plan.md` (exclude `work/items/.archive/`) and filter folder slugs / plan headings containing those keywords. If exactly one match, use it. If multiple matches or the match looks ambiguous (e.g., generic keywords like `review`, `fix`, `update` that could hit many plans), **skip auto-discovery** — a wrong plan is worse than no plan. If zero matches, skip.
...
Read the diff and file list from Stage 1. The 4 always-on personas and 2 SE always-on agents are automatic. For each cross-cutting and stack-specific conditional persona in the persona catalog included below, decide whether the diff warrants it. This is agent judgment, not keyword matching.
...
For SE conditional agents, check if the diff includes files matching `db/migrate/*.rb`, `db/schema.rb`, or data backfill scripts.
...
- se-agent-native-reviewer (always)
- se-learnings-researcher (always)
...
- se-schema-drift-detector -- migration files present
```
...
2. Filter to those whose directory is an ancestor of at least one changed file. A standards file governs all files below it (e.g., `plugins/software-engineering/AGENTS.md` applies to everything under `plugins/software-engineering/`).
...
Three reviewers inherit the session model with no override: `se-correctness-reviewer`, `se-security-reviewer`, and `se-adversarial-reviewer`. These perform the highest-stakes analysis — logic bugs, security vulnerabilities, adversarial failure scenarios — and should run at whatever capability level the user has configured. If the user is on Opus, these get Opus.

All other persona sub-agents and SE agents use the platform's mid-tier model to reduce cost and latency. See the Spawning subsection below for the exact dispatch-time override — the imperative lives there so it lands at the point of action when spawning many agents in parallel.
...
mkdir -p "/tmp/software-engineering/se-code-review/$RUN_ID"
```

Pass `{run_id}` to every persona sub-agent so they can write their full analysis to `/tmp/software-engineering/se-code-review/{run_id}/{reviewer_name}.json`.
...
**Model override at dispatch time.** Pass the platform's mid-tier model on every dispatch except `se-correctness-reviewer`, `se-security-reviewer`, and `se-adversarial-reviewer`, which inherit the session model (per the Model tiering subsection above). In Claude Code, add `model: "sonnet"` to the `Agent` tool call. In Codex, pass the equivalent mid-tier on `spawn_agent` (e.g., `gpt-5.4-mini` as of April 2026). In Pi, pass the equivalent on `subagent` via the `pi-subagents` extension. On platforms where the dispatch primitive has no model-override parameter or the available model names are unknown, omit the override — a working review on the parent model beats a broken dispatch on an unrecognized name. Check this on every Agent / `spawn_agent` / `subagent` call in the parallel dispatch; omitting it on Opus sessions silently 3-4x's the cost of a review.
...
Each persona sub-agent writes full JSON (all schema fields) to `/tmp/software-engineering/se-code-review/{run_id}/{reviewer_name}.json` and returns compact JSON with merge-tier fields only:
...
**SE always-on agents** (se-agent-native-reviewer, se-learnings-researcher) are dispatched as standard Agent calls through the same bounded parallel scheduler as the persona agents. Give them the same review context bundle the personas receive: entry mode, any PR metadata gathered in Stage 1, intent summary, review base branch name when known, `BASE:` marker, file list, diff, and `UNTRACKED:` scope notes. Do not invoke them with a generic "review this" prompt. Their output is unstructured and synthesized separately in Stage 6.

**SE conditional agents** (se-schema-drift-detector, se-deployment-verification-agent) are also dispatched as standard Agent calls through the same bounded parallel scheduler when applicable. Pass the same review context bundle plus the applicability reason (for example, which migration files triggered the agent). For se-schema-drift-detector specifically, pass the resolved review base branch explicitly so it never assumes `main`. Their output is unstructured and must be preserved for Stage 6 synthesis just like the SE always-on agents.

### Stage 5: Merge findings

Convert multiple reviewer compact JSON returns into one deduplicated, confidense-gated finding set. The compact returns contain merge-tier fields (title, severity, file, line, confidence, autofix_class, owner, requires_verification, pre_existing) plus the optional suggested_fix. Detail-tier fields (why_it_matters, evidence) are on disk in the per-agent artifact files and are not loaded at this stage.
...
3. **Spawn validators with bounded parallelism.** One sub-agent per finding, dispatched independently using the validator template and the same bounded scheduler from Stage 4. Each validator receives:
   ...
   - `why_it_matters` when available — loaded from the per-agent artifact file at `/tmp/software-engineering/se-code-review/{run_id}/{reviewer_name}.json`; omit when the file is absent or the artifact write failed. The validator proceeds without it, using the diff and cited code directly.
   - The full diff
...
2. **Findings.** Rendered as pipe-delimited tables grouped by severity (`### P0 -- Critical`, `### P1 -- High`, `### P2 -- Moderate`, `### P3 -- Low`). Each finding row shows `#`, file, issue, reviewer(s), confidence, and synthesized route. Omit empty severity levels. Never render findings as freeform text blocks or numbered lists. Finding numbers come from the stable assignment in Stage 5 -- never re-derive them per severity table.
   ...
   **Pi-native structured emission (interactive mode only):** if the `se_review_finding` tool is registered, also call it once per row in the findings table after rendering the table. Pass `kind: "code"`, the synthesized `severity`, `title`, `file`, `line`, `why_it_matters`, `autofix_class`, `owner`, `requires_verification`, `pre_existing`, `confidence`, `evidence`, and optional `suggested_fix` and `source: "se-code-review:<persona>"`. The harness renders a colored card and persists the finding to the session log so `/se-residuals` and downstream SE skills can read it back without re-parsing the table. Do not emit pre-existing findings or report-only/advisory items twice — the table is canonical; tool emission is additive structured signal, not replacement. Skip this emission entirely in headless and autofix modes — those flows already have artifact files as their machine contract.
...
7. **Learnings & Past Solutions.** Surface se-learnings-researcher results: if past solutions are relevant, flag them as "Known Pattern" with links to docs/solutions/ files.
8. **Agent-Native Gaps.** Surface se-agent-native-reviewer results. Omit section if no gaps found.
9. **Schema Drift Check.** If se-schema-drift-detector ran, summarize whether drift was found. If drift exists, list the unrelated schema objects and the required cleanup command. If clean, say so briefly.
10. **Deployment Notes.** If se-deployment-verification-agent ran, surface the key Go/No-Go items: blocking pre-deploy checks, the most important verification queries, rollback caveats, and monitoring focus areas. Keep the checklist actionable rather than dropping it into Coverage.
...
Artifact: /tmp/software-engineering/se-code-review/<run-id>/
...
**Detail enrichment (headless only):** The headless envelope includes `Why:`, `Evidence:`, and `Suggested fix:` lines. After merge (Stage 5), read the per-agent artifact files from `/tmp/software-engineering/se-code-review/{run_id}/` for only the findings that survived dedup and confidence gating.
...
5. **Protected artifacts are respected.** Discard any findings that recommend deleting or gitignoring files in `work/items/active/*/requirements.md`, `work/items/active/*/plan.md`, or `docs/solutions/`.
...
- **Heterogeneous queue (best-judgment path — interactive option B and walk-through's `Auto-resolve with best judgment on the rest`):** the queue mixes `gated_auto`, `manual`, and `advisory` findings. Each item carries: `autofix_class`, `severity`, `file:line`, `title`, `suggested_fix` (may be null), `why_it_matters`, and `evidence`. The fixer routes each item to one of four buckets — the routing categories are fixed; the failure *reason string* should be specific enough that the post-run question's framing (`N findings could not be auto-resolved...`) reads meaningfully to the user. Use the category's default phrasing below when nothing more specific applies; prefer richer, finding-specific reasons that capture *why this particular item didn't land* (e.g., `needs intent confirmation; was the field narrowing deliberate, or do clients still need the full payload?` is more useful than the generic default).
  ...
  - **Evidense-match check fails:** route to `failed` — default phrasing `evidence no longer matches code at <file:line>` when no richer description fits. This is the false-positive case — the finding cited something that has since changed or was already handled.
...
- In interactive, autofix, and headless modes, write a per-run artifact under `/tmp/software-engineering/se-code-review/<run-id>/` containing:
...
- Also write `metadata.json` alongside the findings so downstream skills (e.g., `se-polish-beta`) can verify the artifact matches the current branch and HEAD. Minimum fields:
  ```json
  {
    "run_id": "<run-id>",
