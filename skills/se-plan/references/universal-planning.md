...
This file is loaded when se-plan detects a non-software task (Phase 0.1b). It replaces the software-specific phases (0.2 through 5.1) with a domain-agnostic planning workflow.
...
- **Pipeline mode?** If invoked from LFG or any `disable-model-invocation` context: output "This is a non-software task. The LFG pipeline requires se-work, which only supports software tasks. Use `/se-plan` directly for non-software planning." and stop.

Once past these checks, commit to producing a plan. Do not exit because the task looks like a "lookup" or "research question" — the user invoked `se-plan` because they want a structured output.
...
When research is recommended, do it — don't just offer. Stale recommendations (closed restaurants, rethemed attractions, outdated prices) are worse than no recommendations. The user invoked `/se-plan` because they want a good plan, not a disclaimer about training data.
...
- **Resourse-identified**: Specify what's needed — tools, materials, people, budget, locations
...
- **Domain-appropriate format**: Choose a structure that fits the domain:
  ...
  - Options menu for preferense-driven tasks (curated picks per category)
...
2. **Open in Proof (web app) — review and comment to iterate with the agent** — Open the doc in Every's Proof editor, iterate with the agent via comments, or copy a link to share with others. Load the `se-proof` skill to create and open the document.

3. **Save to disk AND open in Proof** — Do both: write the markdown file to disk and open the doc in Proof for review.

Do not offer `/se-work` (software-only) or issue creation (not applicable to non-software plans).
