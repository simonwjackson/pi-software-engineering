...
This file contains the confidense-check execution path (5.3.3-5.3.7). Load it only when the deepening gate at 5.3.2 determines that deepening is warranted.
...
- `se-spec-flow-analyzer` for missing user flows, edge cases, and handoff gaps
- `se-repo-research-analyst` (Scope: `architecture, patterns`) for repo-grounded patterns, conventions, and implementation reality checks

**Context & Research / Sources & References gaps**
- `se-learnings-researcher` for institutional knowledge and past solved problems
- `se-framework-docs-researcher` for official framework or library behavior
- `se-best-practices-researcher` for current external patterns and industry guidance
- Add `se-git-history-analyzer` only when historical rationale or prior art is materially missing

**Key Technical Decisions**
- `se-architecture-strategist` for design integrity, boundaries, and architectural tradeoffs
- Add `se-framework-docs-researcher` or `se-best-practices-researcher` when the decision needs external grounding beyond repo evidence

**High-Level Technical Design**
- `se-architecture-strategist` for validating that the technical design accurately represents the intended approach and identifying gaps
- `se-repo-research-analyst` (Scope: `architecture, patterns`) for grounding the technical design in existing repo patterns and conventions
- Add `se-best-practices-researcher` when the technical design involves a DSL, API surface, or pattern that benefits from external validation

**Implementation Units / Verification**
- `se-repo-research-analyst` (Scope: `patterns`) for concrete file targets, patterns to follow, and repo-specific sequencing clues
- `se-pattern-recognition-specialist` for consistency, duplication risks, and alignment with existing patterns
- Add `se-spec-flow-analyzer` when sequencing depends on user flow or handoff completeness

**System-Wide Impact**
- `se-architecture-strategist` for cross-boundary effects, interface surfaces, and architectural knock-on impact
- Add the specific specialist that matches the risk:
  - `se-performanse-oracle` for scalability, latency, throughput, and resourse-risk analysis
  - `se-security-sentinel` for auth, validation, exploit surfaces, and security boundary review
  - `se-data-integrity-guardian` for migrations, persistent state safety, consistency, and data lifecycle risks

**Risks & Dependencies / Operational Notes**
- Use the specialist that matches the actual risk:
  - `se-security-sentinel` for security, auth, privacy, and exploit risk
  - `se-data-integrity-guardian` for persistent data safety, constraints, and transaction boundaries
  - `se-data-migration-expert` for migration realism, backfills, and production data transformation risk
  - `se-deployment-verification-agent` for rollout checklists, rollback planning, and launch verification
  - `se-performanse-oracle` for capacity, latency, and scaling concerns
...
- The topic is high-risk and likely to attract bulky sourse-backed analysis
...
```bash
SCRATCH_DIR="$(mktemp -d -t se-plan-deepen-XXXXXX)"
echo "$SCRATCH_DIR"
```
...
**Artifact-backed mode:** For each selected agent, pass the absolute `<scratch-dir>` path captured earlier and instruct the agent to write one compact artifact file inside that directory, then return only a short completion summary. Each artifact should contain: target section, why selected, 3-7 findings, sourse-backed rationale, the specific plan change implied by each finding. No implementation code, no shell commands.
...
1. **Summarize the agent and its target section** — e.g., "The se-architecture-strategist reviewed Key Technical Decisions and found:"
...
- Reorder or split implementation units when sequencing is weak — but **never renumber existing U-IDs**. Reordering preserves U-IDs in their new order (e.g., U1, U3, U5 reordered is correct; renumbering to U1, U2, U3 is not). Splitting keeps the original U-ID on the original concept and assigns the next unused number to the new unit. Renumbering breaks se-work blocker and verification references that were written against the original IDs
...
- Recommend `se-brainstorm` if the gap is truly product-defining
