...
This content is loaded when a synthesis-summary phase fires in se-plan. There are two variants — they share structure but differ in timing and content focus:
...
- **Out of scope** — deliberately excluded items. Adjacent work the agent considered but decided not to include, refactors, nise-to-haves, future-work items.
...
- AND Phase 0.4 stayed in se-plan (did not route to se-debug, se-work, or universal-planning)
...
Does this match your intent? Tell me what to add, remove, redirect, or that I got wrong — or just confirm to proceed. (You can also redirect to /se-brainstorm if this is bigger than you initially thought — I'll stop here and load it for you.)
```
...
When the skill is invoked from an automated workflow such as LFG or any `disable-model-invocation` context, the skill runs in non-interactive mode (no synchronous user). The artifact is read by downstream skills (se-doc-review, se-work) and human reviewers (PR review).
...
This restores the audit visibility the original design intended (un-validated bets must not propagate as authoritative content), but surfaces them under their own label rather than hiding them. Downstream review (se-doc-review, se-work, human PR review) can scrutinize Assumptions specifically.
...
- **Solo variant**: common redirects include "this is bigger than I thought — let me brainstorm first" (suggest `/se-brainstorm`), "this is just a fix, no plan needed" (suggest `/se-work`), or "I need to investigate first" (suggest `/se-debug`).
- **Brainstorm-sourced variant**: less common, but possible — "actually this scope is wrong, take it back to brainstorm" (suggest `/se-brainstorm` to revise the upstream doc).

In either case: stop se-plan, suggest the alternative skill, offer to load it in-session. Don't push back or argue — the user's redirect signal is the deliberate choice.
