---
name: se-plan
description: "Create structured plans for any multi-step task -- software features, research workflows, events, study plans, or any goal that benefits from structured breakdown. Also deepen existing plans with interactive review of sub-agent findings. Use for plan creation when the user says 'plan this', 'create a plan', 'write a tech plan', 'plan the implementation', 'how should we build', 'what's the approach for', 'break this down', 'plan a trip', 'create a study plan', or when a brainstorm/requirements document is ready for planning. Use for plan deepening when the user says 'deepen the plan', 'deepen my plan', 'deepening pass', or uses 'deepen' in reference to a plan. For exploratory or ambiguous requests where the user is unsure what to do, prefer se-brainstorm first."
argument-hint: "[optional: feature description, requirements doc path, plan path to deepen, or any task to plan]"
compatibility: mise, Rails
---
...
`se-brainstorm` defines **WHAT** to build. `se-plan` defines **HOW** to build it. `se-work` executes the plan. A prior brainstorm is useful context but never required — `se-plan` works from any input: a requirements doc, a bug report, a feature idea, or a rough description.
...
1. **Use requirements as the source of truth** - If `se-brainstorm` produced a requirements document, planning should build from it rather than re-inventing behavior.
...
If the plan already has a `deepened: YYYY-MM-DD` frontmatter field and there is no explicit user request to re-deepen, the fast path still applies the same confidense-gap evaluation — it does not force deepening.
...
- If the ambiguity is mainly product framing, user behavior, or scope definition, recommend `se-brainstorm` as a suggestion — but always offer to continue planning here as well
...
- **Bug-shaped prompt** (user describes broken behavior — "fix the bug where X", error message, regression, "doesn't work"). Surface `se-debug` as a route-out option alongside continuing with `se-plan` whenever the bug surface is reachable (in cwd OR named repo found at another local path). Stay in `se-plan` silently when the named code can't be found anywhere local — paper-planning is the only useful output for unreachable surfaces.
  ...
  **Headless mode**: skip the se-debug suggestion menu entirely; default to continuing with `/se-plan` (the user's explicit invocation). There is no synchronous user to resolve a route-out choice, and auto-routing to se-debug would change the skill mid-flight without authorization.

- **Clear task ready to execute** (known root cause, obvious fix, no architectural decisions) — suggest `se-work` as a faster alternative alongside continuing with planning. The user decides.
...
- Ask the user, using the platform's blocking question tool when available (see Interaction Method), whether to:
  1. Resume `se-brainstorm` to resolve them
...
Fires **only in solo invocation** — when Phase 0.2 found no upstream brainstorm doc AND Phase 0.4 stayed in se-plan (did not route to se-debug, se-work, or universal-planning) AND Phase 0.5 cleared (no unresolved blockers) AND not on Phase 0.1 fast paths (resume normal, deepen-intent). Each guard is an explicit conditional. Skip Phase 0.7 entirely when any guard fails — brainstorm-sourced invocations defer to Phase 5.1.5 instead.
...
- Run subagent with agent="se-repo-research-analyst" and task="Scope: technology, architecture, patterns. {planning context summary}".
- Run subagent with agent="se-learnings-researcher" and task="planning context summary".
Collect:
...
- **Tools available + user asked**: Dispatch `se-slack-researcher` with the planning context summary in parallel with other Phase 1.1 agents. If the origin document has a Slack context section, pass it verbatim so the researcher focuses on gaps. Include findings in consolidation.
...
**Leverage se-repo-research-analyst's technology context:**

The se-repo-research-analyst output includes a structured Technology & Infrastructure summary. Use it to make sharper external research decisions:

- If specific frameworks and versions were detected (e.g., Rails 7.2, Next.js 14, Go 1.22), pass those exact identifiers to se-framework-docs-researcher so it fetches version-specific documentation
...
- Run subagent with agent="se-best-practices-researcher" and task="planning context summary".
- Run subagent with agent="se-framework-docs-researcher" and task="planning context summary".
...
- Run subagent with agent="se-spec-flow-analyzer" and task="planning context summary, research findings".
...
Each unit carries a stable plan-local **U-ID** assigned in Phase 3.5 (`U1`, `U2`, …). U-IDs survive reordering, splitting, and deletion: new units take the next unused number, gaps are fine, and existing IDs are never renumbered. This lets `se-work` reference units unambiguously across plan edits.
...
Distinct from 3.6 (which is about *unknowns* at plan time): 3.7 is about *known but tangential* work that the agent notices while planning but that falls outside the user's confirmed scope. When research surfaces an adjacent refactor, a "while we're here" cleanup, or a scope-adjacent nise-to-have ("we could also add rate limiting"), route it to the existing `### Deferred to Follow-Up Work` subsection in Scope Boundaries (Phase 4.2 Core Plan Template), not into active Implementation Units.

When a deferred item is actionable beyond this plan and should survive independently, use `se-backlog` to capture it rather than relying on the plan prose as the only durable sink.
...
<!-- Include ONLY in non-interactive (headless) mode when the agent had Inferred bets that
     ...
     review (se-doc-review, se-work, human PR review) can scrutinize them as bets, not as
...
<!-- Each unit carries a stable plan-local U-ID (U1, U2, …) assigned sequentially.
     ...
     a gap. This anchor is what se-work references in blockers and verification, so
...
- **Horizontal rules (`---`) between top-level sections** in Standard and Deep plans, mirroring the `se-brainstorm` requirements doc convention. Improves scannability of dense plans where many H2 sections sit close together. Omit for Lightweight plans where the whole doc fits on a single screen.
...
- The plan does not invent product behavior that should have been defined in `se-brainstorm`
...
- Blocking questions were either resolved, explicitly assumed, or sent back to `se-brainstorm`
...
`se-doc-review` and this confidence check are different:
- Use the `se-doc-review` skill when the document needs clarity, simplification, completeness, or scope control
...
1. **Start `/se-work`** (recommended) - Begin implementing this plan in the current session
2. **Run deeper doc review** - Walk through the remaining findings interactively (full se-doc-review walkthrough)
...
- **Start `/se-work`** — Invoke the `se-work` skill via the platform's skill-invocation primitive (`Skill` in Claude Code, `Skill` in Codex, the equivalent on Gemini/Pi), passing the plan path as the skill argument. Do not merely tell the user to type `/se-work` — fire the invocation now so the plan executes in this session.
- **Run deeper doc review** — Re-invoke the `se-doc-review` skill on the plan path **without** `mode:headless` so the interactive routing question and walkthrough fire. After it returns, re-render this menu with refreshed counts so the user can pick a next-stage action.
- **Create Issue** — Detect the project tracker (`gh` for GitHub, `linear` for Linear) and create the issue from the plan file as described under "Issue Creation" in `references/plan-handoff.md`. After creation, display the issue URL and ask whether to proceed to `/se-work` via the platform's blocking question tool.
- **Open in Proof (web app) — review and comment to iterate with the agent** — Load the `se-proof` skill in HITL-review mode with the plan file as `source file`, the plan title as `doc title`, identity `ai:software-engineering` / `Software Engineering`, and recommended next step `/se-work`. Then follow the post-HITL resync logic in `references/plan-handoff.md`, which handles the four `se-proof` return statuses, re-runs `se-doc-review` after material edits, and falls back gracefully on upload failure.
...
**Pipeline mode exception:** In LFG or any `disable-model-invocation` context, skip the interactive menu and return control to the caller after the plan file is written, confidence check has run, and `se-doc-review` has run in headless mode (per `references/plan-handoff.md`).
