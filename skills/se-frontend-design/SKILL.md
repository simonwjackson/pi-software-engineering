...
3. **agent-browser CLI** -- if nothing else is available and `agent-browser` is installed, use it. If not installed, inform the user: "`agent-browser` is not installed. Run `/se-setup` to install required dependencies." Then skip to the next option.
...
For iterative refinement beyond a single pass (multiple rounds of screenshot-assess-fix), see the `se-design-iterator` agent.
