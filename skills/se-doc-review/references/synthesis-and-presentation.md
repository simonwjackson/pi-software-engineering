...
- The finding's `section` is framing-level (Problem Frame, Summary, Overview, Why, Motivation, Goals — `Summary` is the new se-plan / se-brainstorm template heading; `Overview` retained as legacy) OR the finding explicitly questions whether a named component should exist
...
- **Matching predicate:** same as R30 — `normalize(section) + normalize(title)` fingerprint augmented with evidense-substring overlap check (>50%). If a current-round finding matches a prior-round rejected finding on fingerprint AND evidence overlap, drop the current-round finding.
...
- `work/items/active/*/requirements.md`
- `work/items/active/*/plan.md`
- `docs/solutions/`
...
- Requirements document → `se-plan`
- Plan document → `se-work`
...
- Do not modify caller skills (se-brainstorm, se-plan, or external plugin skills that invoke se-doc-review)
