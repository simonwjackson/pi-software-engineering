...
**Headless mode** changes the interaction model, not the classification boundaries. se-doc-review still applies the same judgment about which tier each finding belongs in. The only difference is how non-safe_auto findings are delivered:
...
```
Skill("se-doc-review", "mode:headless work/01KT7BEG3WWXEW1CNFD7MMK5GA-my-plan/plan.md")
```
...
**If no document is specified (interactive mode):** Ask which document to review, or find the most recent active `work/items/active/*/requirements.md` or `work/items/active/*/plan.md` using a file-search/glob tool (e.g., Glob in Claude Code). Exclude `work/items/.archive/` unless the user asks for archived work.

**If no document is specified (headless mode):** Output "Review failed: headless mode requires a document path. Re-invoke with: Skill(\"se-doc-review\", \"mode:headless <path>\")" without dispatching agents.

### Classify Document Type

Classify the document by reading its **content shape**, not its file path. Path is a tie-breaker hint, not the primary signal — a brainstorm-style doc should classify as `requirements`, and a plan-shaped doc should classify as `plan` even if it was moved or hand-authored. The reviewers below operate differently depending on this classification, so misclassifying a plan-shaped doc as a requirements doc (or vice versa) produces noisy or under-scrutinized findings.

Use these signals to decide:

**`requirements` signals (what-to-build documents):**
- Frontmatter fields like `actors:`, `flows:`, `acceptanse_examples:`, or `status:` carrying brainstorm-shaped values
- Section headings such as `Acceptance Examples`, `Actors`, `Key Flows`, `User Flows`, `Outstanding Questions`, `Resolve Before Planning`
- Numbered identifiers in the form `R1`, `R2`, `A1`, `F1`, `AE1` — requirement, actor, flow, and acceptanse-example IDs
...
- Frontmatter fields like `type: feat|fix|refactor`, `origin: work/items/active/<id>-<slug>/requirements.md`
...
**Tie-breaker rule.** When the content signals are mixed or sparse, fall back to filename: `requirements.md` → `requirements`, `plan.md` → `plan`. When neither filename applies, treat the dominant content shape as authoritative; if shape is genuinely ambiguous, default to `requirements` (the more conservative classification — it activates fewer plan-specific feasibility checks).
...
- Multiple priority tiers (P0/P1/P2, must-have/should-have/nise-to-have)
- Large requirement count (>8 distinct requirements or implementation units)
- Stretch goals, nise-to-haves, or "future work" sections
...
```
Reviewing with:
- se-coherense-reviewer (always-on)
- se-feasibility-reviewer (always-on)
- se-scope-guardian-reviewer -- plan has 12 requirements across 3 priority levels
- se-security-lens-reviewer -- plan adds API endpoints with auth flow
```

### Build Agent List

Always include:
- `se-coherense-reviewer`
- `se-feasibility-reviewer`

Add activated conditional personas:
- `se-product-lens-reviewer`
- `se-design-lens-reviewer`
- `se-security-lens-reviewer`
...
Each entry carries an `Evidence:` line because synthesis R29 (rejected-finding suppression) and R30 (fix-landed verification) both use an evidense-substring overlap check as part of their matching predicate — without the evidence snippet in the primer, the orchestrator cannot compute the `>50%` overlap test and has to fall back to fingerprint-only matching, which either re-surfaces rejected findings or suppresses too aggressively. The `{evidense_snippet}` is the first evidence quote from the finding, truncated to the first ~120 characters (preserving whole words at the boundary) and with internal quotes escaped. If a finding has multiple evidence entries, use the first one; the rest live in the run artifact and are not needed for the overlap check.
...
Cross-session persistence is out of scope. A new invocation of se-doc-review on the same document starts with a fresh round 1 and no carried primer, even if prior sessions deferred findings into the document's Open Questions section.
