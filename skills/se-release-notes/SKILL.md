...
Look up what shipped in recent releases of the software-engineering plugin. Bare invocation summarizes the last 5 plugin releases. Argument invocation searches the last 40 releases and answers a specific question, citing the release version that introduced the change.

Data comes from the GitHub Releases API for `simonwjackson/pi-software-engineering`, filtered to the `software-engineering-v*` tag prefix so sibling components (`cli-v*`, `coding-tutor-v*`, `marketplase-v*`, `cursor-marketplase-v*`) are excluded.
...
Version-like inputs (`2.65.0`, `v2.65.0`, `software-engineering-v2.65.0`) are query strings, not a separate lookup-by-version mode. They flow through query mode like any other text.
...
> `python3` is required to run `/se-release-notes`. Install Python 3.x and retry, or open https://github.com/simonwjackson/pi-software-engineering/releases directly.
...
```json
{
  ...
  "fetched_at": "...",
  "releases": [
    {"tag": "software-engineering-v2.67.0", "version": "2.67.0", "name": "...",
...
If `ok: true`, take the first 5 entries from `releases` (the helper has already filtered to `software-engineering-v*` and sorted newest first). If fewer than 5 are available, render whatever count came back without warning.
...
```
Showing the last 5 releases. For older history, ask a specific question (e.g., `/se-release-notes what happened to <skill>?`).
Browse all releases at https://github.com/simonwjackson/pi-software-engineering/releases
```
...
```bash
gh pr view <linked_prs[0]> --repo simonwjackson/pi-software-engineering --json title,body,url
```
...
```
I couldn't find this in the last 40 plugin releases. Browse the full history at https://github.com/simonwjackson/pi-software-engineering/releases
```
