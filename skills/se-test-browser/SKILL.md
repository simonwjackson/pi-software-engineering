...
If not installed, inform the user: "`agent-browser` is not installed. Run `/se-setup` to install required dependencies." Then stop — this skill cannot function without agent-browser.
...
If not installed, inform the user: "`agent-browser` is not installed. Run `/se-setup` to install required dependencies." Then stop.
...
/se-test-browser feature/new-dashboard

# Test on a specific port
/se-test-browser --port 5000
```
