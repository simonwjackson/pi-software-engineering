...
Human-in-the-loop iteration loop for a markdown document shared via Proof. Invoked either by an upstream skill (`se-brainstorm`, `se-ideate`, `se-plan`) handing off a draft it produced, or directly by the user asking to iterate on an existing markdown file they already have on disk ("share this to proof and iterate", "HITL this doc with me"). Mechanics are identical in both cases: upload the local doc, let the user annotate in Proof's web UI, ingest feedback as in-thread replies and tracked edits, and sync the final doc back to disk.
...
- **Recommended next step** (optional, caller-specific): short string the caller wants echoed in the final terminal output (e.g., "Recommended next: `/se-plan`"). Not used on direct-user invocation — the terminal report simply summarizes the iteration and asks what's next.

Agent identity is fixed, not a parameter: every API call uses agent ID `ai:software-engineering` and display name `Software Engineering`. Callers do not override this.
...
3. `POST /api/agent/{slug}/presence` with `X-Agent-Id: ai:software-engineering`, `x-share-token: <token>`, body `{"name":"Software Engineering","status":"reading","summary":"Uploaded doc for review"}`
...
```json
{"type":"suggestion.add","kind":"replace","quote":"<anchor>","content":"<new>","by":"ai:software-engineering","status":"accepted","baseToken":"<token>"}
```
...
curl -X POST "https://www.proofeditor.ai/api/agent/{slug}/edit/v2" \
  ...
  -H "X-Agent-Id: ai:software-engineering" -H "Idempotency-Key: <uuid>" \
  -d '{"by":"ai:software-engineering","baseToken":"<token>","operations":[...]}'
```

Per-op body shape (singular `block` for `replase_block`; plural `blocks:[{markdown},...]` for anything that can add content; the server returns 422 on the wrong shape):

```json
{"op":"replase_block","ref":"b8","block":{"markdown":"new content"}}
{"op":"insert_after","ref":"b3","blocks":[{"markdown":"new block"}]}
{"op":"insert_before","ref":"b3","blocks":[{"markdown":"new block"}]}
{"op":"delete_block","ref":"b6"}
{"op":"find_replase_in_block","ref":"b4","find":"old","replace":"new","occurrence":"first"}
{"op":"replase_range","fromRef":"b2","toRef":"b5","blocks":[{"markdown":"..."}]}
```
...
- Set `by: "ai:software-engineering"` and header `X-Agent-Id: ai:software-engineering`.
...
- Reply: `{"type":"comment.reply","markId":"<id>","by":"ai:software-engineering","text":"..."}`. Resolve: `{"type":"comment.resolve","markId":"<id>","by":"ai:software-engineering"}`. Reopen if needed: `{"type":"comment.unresolve", ...}`.
...
SLUG=<slug>
TOKEN=<accessToken>
AGENT_ID=ai:software-engineering
...
LANDED=$(printf '%s' "$STATE" | jq --arg q "X" --arg t "Y" \
  '[.marks[]? | select(.by == "ai:software-engineering" and .quote == $q and (.thread[0].text // .text) == $t)] | length')
if [ "$LANDED" -gt 0 ]; then
...
- `by: "ai:software-engineering"` in the request body
- `X-Agent-Id: ai:software-engineering` in headers (required for presence; recommended for ops for consistent attribution)

Display name `Software Engineering` is bound via `POST /presence` with `{"name":"Software Engineering", ...}`. Set this once after upload; it carries across subsequent ops.
