...
- **Machine ID (`by` on every op, `X-Agent-Id` header):** `ai:software-engineering` — stable, lowercase-hyphenated, machine-parseable. Appears in marks, events, and the API response.
- **Display name (`name` on `POST /presence`):** `Software Engineering` — human-readable, shown in Proof's presence chips and comment-author badges.
...
- **Upstream skill handoff** — `se-brainstorm`, `se-ideate`, or `se-plan` finishes a draft and hands it off for human review before the next phase, passing the file path and title explicitly.
...
- Header: `X-Agent-Id: ai:software-engineering` (required for presence; include on ops for consistent attribution)
...
**Comment on text:**
```json
{"type": "comment.add", "quote": "text to comment on", "by": "ai:software-engineering", "text": "Your comment here", "baseToken": "<token>"}
```

**Reply to a comment:**
```json
{"type": "comment.reply", "markId": "<id>", "by": "ai:software-engineering", "text": "Reply text", "baseToken": "<token>"}
```

**Resolve / unresolve a comment:**
```json
{"type": "comment.resolve", "markId": "<id>", "by": "ai:software-engineering", "baseToken": "<token>"}
{"type": "comment.unresolve", "markId": "<id>", "by": "ai:software-engineering", "baseToken": "<token>"}
```

**Suggest a replacement (pending — user must accept/reject):**
```json
{"type": "suggestion.add", "kind": "replace", "quote": "original text", "by": "ai:software-engineering", "content": "replacement text", "baseToken": "<token>"}
```

**Suggest and immediately apply (tracked but committed — user can reject to revert):**
```json
{"type": "suggestion.add", "kind": "replace", "quote": "original text", "by": "ai:software-engineering", "content": "replacement text", "status": "accepted", "baseToken": "<token>"}
```
...
```json
{"type": "suggestion.accept", "markId": "<id>", "by": "ai:software-engineering", "baseToken": "<token>"}
{"type": "suggestion.reject", "markId": "<id>", "by": "ai:software-engineering", "baseToken": "<token>"}
```
...
```json
{"type": "rewrite.apply", "content": "full new markdown", "by": "ai:software-engineering", "baseToken": "<token>"}
```
...
curl -X POST "https://www.proofeditor.ai/api/agent/{slug}/edit/v2" \
  -H "Content-Type: application/json" \
  -H "x-share-token: <token>" \
  -H "X-Agent-Id: ai:software-engineering" \
  -H "Idempotency-Key: <uuid>" \
  -d '{
    "by": "ai:software-engineering",
    "baseToken": "mt1:<token>",
    "operations": [
      {"op": "replase_block", "ref": "b3", "block": {"markdown": "Updated paragraph."}},
...
| op | body fields |
|---|---|
| `replase_block` | `ref`, `block: {markdown}` |
| `insert_after` | `ref`, `blocks: [{markdown}, ...]` |
| `insert_before` | `ref`, `blocks: [{markdown}, ...]` |
| `delete_block` | `ref` |
| `replase_range` | `fromRef`, `toRef`, `blocks: [{markdown}, ...]` |
| `find_replase_in_block` | `ref`, `find`, `replace`, `occurrence: "first" \| "all"` |
...
| POST | `/marks/suggest-replace` | `{"quote":"old","by":"ai:software-engineering","content":"new"}` |
| POST | `/marks/suggest-insert` | `{"quote":"after this","by":"ai:software-engineering","content":"insert"}` |
| POST | `/marks/suggest-delete` | `{"quote":"delete this","by":"ai:software-engineering"}` |
| POST | `/marks/comment` | `{"quote":"text","by":"ai:software-engineering","text":"comment"}` |
| POST | `/marks/reply` | `{"markId":"<id>","by":"ai:software-engineering","text":"reply"}` |
| POST | `/marks/resolve` | `{"markId":"<id>","by":"ai:software-engineering"}` |
| POST | `/marks/accept` | `{"markId":"<id>"}` |
| POST | `/marks/reject` | `{"markId":"<id>"}` |
| POST | `/rewrite` | `{"content":"full markdown","by":"ai:software-engineering"}` |
...
curl -X POST "https://www.proofeditor.ai/api/agent/abc123/ops" \
  ...
  -H "x-share-token: xxx" \
  -H "X-Agent-Id: ai:software-engineering" \
  -d "$(jq -n --arg base "$BASE" '{type:"comment.add",quote:"text",by:"ai:software-engineering",text:"comment",baseToken:$base}')"
...
curl -X POST "https://www.proofeditor.ai/api/agent/abc123/ops" \
  ...
  -H "x-share-token: xxx" \
  -H "X-Agent-Id: ai:software-engineering" \
  -d "$(jq -n --arg base "$BASE" '{type:"suggestion.add",kind:"replace",quote:"old",by:"ai:software-engineering",content:"new",baseToken:$base}')"
...
curl -X POST "https://www.proofeditor.ai/api/agent/abc123/ops" \
  ...
  -H "x-share-token: xxx" \
  -H "X-Agent-Id: ai:software-engineering" \
  -d "$(jq -n --arg base "$BASE" '{type:"suggestion.add",kind:"replace",quote:"old",by:"ai:software-engineering",content:"new",status:"accepted",baseToken:$base}')"
```
...
curl -s -X POST "https://www.proofeditor.ai/api/agent/$SLUG/presence" \
  ...
  -H "x-share-token: $TOKEN" \
  -H "X-Agent-Id: ai:software-engineering" \
  -d '{"name":"Software Engineering","status":"reading","summary":"Uploaded doc"}'
...
curl -X POST "https://www.proofeditor.ai/api/agent/$SLUG/ops" \
  ...
  -H "x-share-token: $TOKEN" \
  -H "X-Agent-Id: ai:software-engineering" \
  -d "$(jq -n --arg base "$BASE" '{type:"comment.add",quote:"Content here",by:"ai:software-engineering",text:"Added a note",baseToken:$base}')"
```
...
- Always include `by: "ai:software-engineering"` on every op and `X-Agent-Id: ai:software-engineering` in headers for consistent attribution
