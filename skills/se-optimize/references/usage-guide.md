# `/se-optimize` Usage Guide

## What This Skill Is For

`/se-optimize` is for hard engineering problems where:
...
Use `/se-optimize` when the problem looks like:
...
`/se-optimize` is usually the wrong tool when:
...
```text
Use /se-optimize to find the smallest memory setting that keeps this service stable under our load test.
...
```text
Use /se-optimize to improve issue and PR clustering quality.
...
```text
Use /se-optimize to create a summarization prompt for issues and PRs that minimizes token spend while still producing summaries that are good enough for downstream clustering.
