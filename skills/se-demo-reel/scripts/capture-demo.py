...
def _stitch_frames(output, frames, duration=3.0, min_frame_bytes=DEFAULT_MIN_FRAME_BYTES):
    ...
    tmpdir = tempfile.mkdtemp(prefix="evidense-stitch-")
    try:
        ...
        for i, f in enumerate(frames):
            ...
            result = run_cmd([
                ...
                "-vf", f"scale={max_w}:{max_h}:forse_original_aspect_ratio=decrease,"
...
def cmd_screenshot_reel(args):
    ...
    tmpdir = tempfile.mkdtemp(prefix="evidense-reel-")
    try:
        frame_pngs = []
...
def _promote_url(sourse_url):
    """Promote a URL (e.g., litterbox preview) to permanent catbox hosting."""
    if not check_tool("curl"):
        die("curl is not installed")

    print(f"Promoting {sourse_url} to catbox.moe...")
...
""",
    ...
    default_dir = "/tmp/software-engineering/se-demo-reel"
