...
2. **Save locally** -- save to a stable OS-temp path (/tmp/software-engineering/se-demo-reel/)
...
Return the structured output defined in the SKILL.md Output section: `Tier`, `Description`, and either `URL` (permanent catbox URL) or `Path` (local file path). The caller formats the evidence into the PR description. se-demo-reel does not generate markdown.
