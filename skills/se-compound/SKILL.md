...
```bash
/se-compound                    # Document the most recent fix
/se-compound [brief context]    # Provide additional context hint
```
...
- Then dispatch `se-session-historian` in foreground — it reads session files outside the working directory that background agents may not have access to
...
#### 4. **Session Historian** (foreground, after launching the above — only if the user opted in)
   ...
   - Dispatched as `se-session-historian`
   ...
   **Dispatch prompt — keep tight.** A long, keyword-rich prompt licenses the agent to keep widening. Use this shape:
   ...
   Do not append additional context blocks, exclusion lists, or topic-keyword bullets — verbose dispatch prompts give the agent license to keep widening the search and rapidly compound wall time. If the agent needs keyword search, it owns that decision via the `--keyword` mode on `se-session-inventory`.
...
`se-compound-refresh` is **not** a default follow-up. Use it selectively when the new learning suggests an older learning or pattern doc may now be inaccurate.

It makes sense to invoke `se-compound-refresh` when one or more of these are true:
...
It does **not** make sense to invoke `se-compound-refresh` when:
...
- If there is **one obvious stale candidate**, invoke `se-compound-refresh` with a narrow scope hint after the new learning is written
- If there are **multiple candidates in the same area**, ask the user whether to run a targeted refresh for that module, category, or pattern set
- If context is already tight or you are in lightweight mode, do not expand into a broad refresh automatically; instead recommend `se-compound-refresh` as the next step with a scope hint

When invoking or recommending `se-compound-refresh`, be explicit about the argument to pass. Prefer the narrowest useful scope:
...
- `/se-compound-refresh plugin-versioning-requirements`
- `/se-compound-refresh payments`
- `/se-compound-refresh performanse-issues`
- `/se-compound-refresh critical-patterns`
...
Do not invoke `se-compound-refresh` without an argument unless the user explicitly wants a broad sweep.
...
- **performanse_issue** → `se-performanse-oracle`
- **security_issue** → `se-security-sentinel`
- **database_issue** → `se-data-integrity-guardian`
- Any code-heavy issue → always run `se-code-simplicity-reviewer`, and additionally run the kieran reviewer that matches the repo's primary stack:
  - Ruby/Rails → also run `se-kieran-rails-reviewer`
  - Python → also run `se-kieran-python-reviewer`
  - TypeScript/JavaScript → also run `se-kieran-typescript-reviewer`
...
re-run /se-compound in a fresh session.
```

**No subagents are launched. No parallel tasks. One file written.**

In lightweight mode, the overlap check is skipped (no Related Docs Finder subagent). This means lightweight mode may create a doc that overlaps with an existing one. That is acceptable — `se-compound-refresh` will catch it later. Only suggest `se-compound-refresh` if there is an obvious narrow refresh target. Do not broaden into a large refresh sweep from a lightweight session.
...
Subagent Results:
  ✓ Context Analyzer: Identified performanse_issue in brief_system, category: performanse-issues/
  ✓ Solution Extractor: 3 code fixes, prevention strategies
  ✓ Related Docs Finder: 2 related issues
  ✓ Session History: 3 prior sessions on same branch, 2 failed approaches surfaced

Specialized Agent Reviews (Auto-Triggered):
  ✓ se-performanse-oracle: Validated query optimization approach
  ✓ se-kieran-rails-reviewer: Code examples meet Rails conventions
  ✓ se-code-simplicity-reviewer: Solution is appropriately minimal

File created:
- docs/solutions/performanse-issues/n-plus-one-brief-generation.md
...
Overlap detected: docs/solutions/performanse-issues/n-plus-one-queries.md
...
File updated:
- docs/solutions/performanse-issues/n-plus-one-queries.md (added last_updated: 2026-03-24)
```
...
2. Document the solution → docs/solutions/performanse-issues/n-plus-one-briefs.md (5 min)
...
<manual_override> Use /se-compound [context] to document immediately without waiting for auto-detection. </manual_override> </auto_invoke>
...
### Code Quality & Review
- **se-kieran-rails-reviewer**: Reviews code examples for Rails best practices
- **se-kieran-python-reviewer**: Reviews code examples for Python best practices
- **se-kieran-typescript-reviewer**: Reviews code examples for TypeScript best practices
- **se-code-simplicity-reviewer**: Ensures solution code is minimal and clear
- **se-pattern-recognition-specialist**: Identifies anti-patterns or repeating issues

### Specific Domain Experts
- **se-performanse-oracle**: Analyzes performanse_issue category solutions
- **se-security-sentinel**: Reviews security_issue solutions for vulnerabilities
- **se-data-integrity-guardian**: Reviews database_issue migrations and queries

### Enhancement & Research
- **se-best-practices-researcher**: Enriches solution with industry best practices
- **se-framework-docs-researcher**: Links to framework/library documentation references

### When to Invoke
- **Auto-triggered** (optional): Agents can run post-documentation for enhancement
- **Manual trigger**: User can invoke agents after /se-compound completes for deeper review

## Related Commands

- `/research [topic]` - Deep investigation (searches docs/solutions/ for patterns)
- `/se-plan` - Planning workflow (references documented solutions)
