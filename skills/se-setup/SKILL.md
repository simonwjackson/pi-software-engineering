...
Interactive setup for software-engineering — diagnoses environment health, cleans obsolete repo-local SE config, and helps configure required tools. Review agent selection is handled automatically by `se-code-review`; project-specific review guidance belongs in `CLAUDE.md` or `AGENTS.md`.

## Phase 1: Diagnose

### Step 1: Determine Plugin Version

Detect the installed software-engineering plugin version by reading the plugin metadata or manifest. This is platform-specific -- use whatever mechanism is available (e.g., reading `plugin.json` from the plugin root or cache directory). If the version cannot be determined, skip this step.

If a version is found, pass it to the check script via `--version`. Otherwise omit the flag.

### Step 2: Run the Health Check Script

Before running the script, display: "Software Engineering -- checking your environment..."

Run the bundled check script. Do not perform manual dependency checks -- the script handles all CLI tools, agent skills, repo-local SE file checks, and `.gitignore` guidance in one pass.
...
If the line above resolved to an absolute path (starts with `/` and contains no `${`), this is a Claude Code session and `/se-update` is available. Anything else — empty, the literal `${CLAUDE_PLUGIN_ROOT}` token, or an unresolved command string like `echo "${CLAUDE_PLUGIN_ROOT}"` left in place by a non-Claude harness that doesn't process `!` pre-resolution — means this is not Claude Code; omit any `/se-update` references from output.
...
- `software-engineering.local.md` is present and needs cleanup
- `.software-engineering/config.local.yaml` does not exist or is not safely gitignored
- `.software-engineering/config.local.example.yaml` is missing or outdated

If everything is installed, no repo-local cleanup is needed, and `.software-engineering/config.local.yaml` already exists and is gitignored, display the tool and skill list and completion message. Parse the tool and skill names from the script output and list each with a green circle. Omit the Skills line if the Skills section is absent from the script output:

```
 ✅ Software Engineering setup complete

    Tools:  🟢 agent-browser  🟢 gh  🟢 jq  🟢 vhs  🟢 silicon  🟢 ffmpeg  🟢 ast-grep
    Skills: 🟢 ast-grep
    Config: ✅

    Run /se-setup anytime to re-check.
```

If this is a Claude Code session (the **Plugin root** above resolved to a non-empty path), append to the message: "Run /se-update to grab the latest plugin version."
...
### Step 4: Resolve Repo-Local SE Issues

Resolve the repository root (`git rev-parse --show-toplevel`). If `software-engineering.local.md` exists at the repo root, explain that it is obsolete because review-agent selection is automatic and SE now uses `.software-engineering/config.local.yaml` for any surviving machine-local state. Ask whether to delete it now. Use the repo-root path when deleting.
...
**Example file (always refresh):** Copy `references/config-template.yaml` to `<repo-root>/.software-engineering/config.local.example.yaml`, creating the directory if needed. This file is committed to the repo and always overwritten with the latest template so teammates can see available settings.

**Local config (create once):** If `.software-engineering/config.local.yaml` does not exist, ask whether to create it:

```
Set up a local config file for this project?
This saves your Software Engineering preferences (like which tools to use and how workflows behave).
...
If the user approves, copy `references/config-template.yaml` to `<repo-root>/.software-engineering/config.local.yaml`. If `.software-engineering/config.local.yaml` is not already covered by `.gitignore`, offer to add the entry:

```text
.software-engineering/*.local.yaml
```
...
If this is a Claude Code session (per platform detection in Step 3), append: "Run /se-update to grab the latest plugin version."
